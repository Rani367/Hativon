...
// Extend bun's expect with jest-dom matchers (toBeInTheDocument, toHaveAttribute, etc.)
expect.extend(jestDomMatchers);

// ---------- DOM environment (happy-dom) ----------
// Register DOM globals so component tests using @testing-library/react work.
...
// ---------- Global delegate system ----------
// Some modules are mocked by multiple test files. Since bun's mock.module is
// process-global (last call wins), competing definitions break other files.
// The delegate pattern solves this: setup.ts registers ONE mock.module per
// contested path, and individual test files swap the delegate in beforeEach.
const _g = globalThis as Record<string, unknown>;

// Initialise delegate slots
...
// Note: @/lib/rate-limit is NOT mocked here because rate-limit.test.ts needs the
// real module. Each consuming test file should provide its own complete mock.

// ---------- Mock: @/lib/posts/queries ----------
// This sub-module is mocked by storage tests (which only need getPostById) and
// also loaded by performance tests (which need all exports). A single mock with
// delegates prevents partial mocks from contaminating other files.
//
// Load the REAL queries module BEFORE mocking, so performance tests can use it.
// Capture individual function references (not the module object) to avoid them
// being replaced when mock.module overwrites exports in-place.
...
// Default delegates forward to the real implementations
...
// ---------- Mock: @/lib/posts (barrel) ----------
// Must provide ALL exports from the barrel to prevent incomplete mocks
