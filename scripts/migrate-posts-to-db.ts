  ...
  try {
    if (isVercel) {
      ...
      try {
        ...
        if (!response.ok) {
          console.log('[WARNING]  No posts found in Blob storage');
          return [];
        }

        const text = await response.text();
        const posts = JSON.parse(text);
        console.log(`[OK] Found ${posts.length} posts in Blob storage`);
        return posts;
      } catch (error) {
        console.log('[WARNING]  No posts found in Blob storage (blob may not exist yet)');
        return [];
      }
    } else {
      console.log('📥 Reading posts from local JSON file...');
      if (!existsSync(LOCAL_DATA_FILE)) {
        console.log('[WARNING]  No local posts.json file found');
        return [];
      }

      const data = readFileSync(LOCAL_DATA_FILE, 'utf-8');
      const posts = JSON.parse(data);
      console.log(`[OK] Found ${posts.length} posts in local file`);
      return posts;
    }
  } catch (error) {
    console.error('[ERROR] Error reading old posts:', error);
    return [];
  }
}
...
async function migratePostToDatabase(post: Post): Promise<boolean> {
  try {
    ...
    console.error(`  [ERROR] Error migrating post "${post.title}":`, error);
    return false;
  }
}

async function main() {
  console.log('[SETUP] Starting posts migration to PostgreSQL...\n');
  ...
  try {
    ...
    if (!tableCheck.rows[0]?.exists) {
      console.error('[ERROR] Posts table does not exist!');
    ...
    if (posts.length === 0) {
      console.log('\n[OK] No posts to migrate. Database is ready for new posts!');
      process.exit(0);
    }

    // Migrate each post
    console.log(`\n[INFO] Migrating ${posts.length} posts to PostgreSQL...\n`);
    let successCount = 0;
    let skipCount = 0;
    ...
    console.log(`   [OK] Successfully migrated: ${successCount} posts`);
    if (skipCount > 0) {
      console.log(`   [WARNING]  Skipped (already exist): ${skipCount} posts`);
    }
