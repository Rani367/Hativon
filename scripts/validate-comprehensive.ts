...
function printResults() {
  ...
  console.log(
    `${colors.red}Failed: ${totalChecks - passedChecks}${colors.reset}`,
  );
  console.log(
    `${colors.red}${colors.bold}Critical Failures: ${criticalFailures}${colors.reset}`,
  );
  console.log(`${"=".repeat(80)}\n`);

  if (criticalFailures > 0) {
    console.log(
      `${colors.red}${colors.bold}[ERROR] ${criticalFailures} CRITICAL FAILURE(S) DETECTED${colors.reset}`,
    );
    console.log(
    ...
    console.log(
      `${colors.yellow}[WARNING] ${totalChecks - passedChecks} non-critical issues found${colors.reset}`,
    );
    console.log(
